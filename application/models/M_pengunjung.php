...
class M_pengunjung extends CI_Model
{
    ...
    function kunjungan()
    {
        ...
        $tanggal = date("Y-m-d");
        $waktu   = time();
        $cekk = $this->db->query("SELECT * FROM tbl_pengunjung WHERE pengunjung_ip='$ip' AND tanggal='$tanggal'");
        $rowh = $cekk->row_array();
        if ($cekk->num_rows() == 0) {
            $datadb = array('pengunjung_ip' => $ip, 'tanggal' => $tanggal, 'hits' => '1', 'online' => $waktu);
            $this->db->insert('tbl_pengunjung', $datadb);
        } else {
            $hitss = $rowh['hits'] + 1;
            $datadb = array('pengunjung_ip' => $ip, 'tanggal' => $tanggal, 'hits' => $hitss, 'online' => $waktu);
    ...
    {
        $bataswaktu       = time() - 300;
        $hsl = $this->db->query("SELECT * FROM tbl_pengunjung WHERE online > '$bataswaktu'");
        return $hsl;
    }
}
